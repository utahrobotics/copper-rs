...
fn gen_culist_support(
        ...
        if pack.is_multi() {
            for port_idx in 0..pack.msg_types.len() {
                ...
                zeroed_init_tokens.push(quote! {
                    ...
                    self.0.#slot_index.#port_index.clear_payload();
                });
            }
        } else {
            zeroed_init_tokens.push(quote! {
                self.0.#slot_index.metadata.status_txt = CuCompactString::default();
                self.0.#slot_index.clear_payload();
            });
        }
    }
    ...
    // Generate helper methods for bridge rx channels (they produce outputs like source tasks)
    let bridge_rx_methods: Vec<_> = bridge_specs.iter().flat_map(|spec| {
        spec.rx_channels.iter().map(|channel| {
            let culist_index = channel
                .culist_index
                .expect("Bridge Rx channel should have culist_index set");
            let fn_name = format_ident!(
                "get_{}_rx_{}_output",
                config_id_to_struct_member(&spec.id),
                config_id_to_struct_member(&channel.id)
            );
            let slot_index = syn::Index::from(culist_index);
            let pack = &output_packs[culist_index];
            ...
            if pack.msg_types.len() == 1 {
                ...
                // For multi-port bridges, find the correct port index
                let port_index = pack.msg_types.iter().position(|t| {
                    format!("{}", quote!(#t)) == channel.msg_type_name
                }).expect("Bridge Rx channel message type not found in output pack");
                let payload_type = &pack.msg_types[port_index];
                let port_idx = syn::Index::from(port_index);
                quote! {
                    #[allow(dead_code)]
                    pub fn #fn_name(&self) -> &CuMsg<#payload_type> {
                        &self.0.#slot_index.#port_idx
                    }
                }
            }
        }).collect::<Vec<_>>()
            ...
            CuExecutionUnit::Step(step) => {
                ...
                match &exec_entities[step.node_id as usize].kind {
                    ...
                    ExecutionEntityKind::BridgeRx { bridge_index, channel_index } => {
                        ...
                        let _channel = &bridge_spec.rx_channels[*channel_index];
                        let output_pack = step.output_msg_pack.as_ref()
                            .expect("Bridge Rx channel should have output pack");
                        let msg_types: Vec<Type> = output_pack
                            ...
                            .collect();
                        let output = build_output_slot_type(&msg_types);
                        
                        Some(quote! {
                            #enum_ident(CuTaskCallbackState<(), &'a mut #output>)
                        })
                    }
                    ExecutionEntityKind::BridgeTx { bridge_index, channel_index } => {
                        let bridge_spec = &bridge_specs[*bridge_index];
                        let _channel = &bridge_spec.tx_channels[*channel_index];
                        let input = step.input_msg_indices_types.first()
                            .expect("Bridge Tx channel should have input");
                        let input_type = parse_str::<Type>(format!("CuMsg<{}>", input.msg_type).as_str()).unwrap();
                        let no_output = parse_str::<Type>("CuMsg<()>").unwrap();
                        
                        Some(quote! {
                            #enum_ident(CuTaskCallbackState<&'a #input_type, &'a mut #no_output>)
                        })
                    }
                }
            }
...
pub fn copper_runtime(args: TokenStream, input: TokenStream) -> TokenStream {
    ...
    for (mission, graph) in &all_missions {
        ...
        let sim_support = if sim_mode {
            Some(gen_sim_support(&culist_plan, &culist_exec_entities, &culist_bridge_specs))
        } else {
            None
        };
...
fn generate_bridge_rx_execution_tokens(
    ...
    let enum_name = Ident::new(&config_id_to_enum(step.node.get_id().as_str()), Span::call_site());
    ...
    let call_sim_callback = if sim_mode {
        quote! {
            let doit = {
                ...
                let mut dummy_output: #no_output = Default::default();
                let state = CuTaskCallbackState::Process(cumsg_input, &mut dummy_output);
                let ovr = sim_callback(SimStep::#enum_name(state));
